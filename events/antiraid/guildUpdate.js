				...
				if (db.get(`update_${guild.id}`) === true && !perm) {
					if (db.get(`updatesanction_${guild.id}`) === "ban") {
						...
						axios({
							...
							if (raidlog) return raidlog.send(new EmbedBuilder().setColor(color).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a modifier le serveur, il a été **ban** !`))
						}).catch(() => {
							update(oldGuild, newGuild)
							if (raidlog) return raidlog.send(new EmbedBuilder().setColor(color).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a modifier le serveur, mais il n'a pas pu être **ban** !`))

						})
					} else if (db.get(`updatesanction_${guild.id}`) === "kick") {
						guild.members.cache.get(response.data.audit_log_entries[0].user_id).kick().then(() => {
							update(oldGuild, newGuild)
							if (raidlog) return raidlog.send(new EmbedBuilder().setColor(color).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a modifier le serveur, il a été **kick** !`))
						}).catch(() => {
							update(oldGuild, newGuild)
							if (raidlog) return raidlog.send(new EmbedBuilder().setColor(color).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a modifier le serveur, mais il n'a pas pu être **kick** !`))
						})
						...
						guild.members.cache.get(response.data.audit_log_entries[0].user_id).roles.set([]).then(() => {
							...
							update(oldGuild, newGuild)
							if (raidlog) return raidlog.send(new EmbedBuilder().setColor(color).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a modifier le serveur, il a été **derank** !`))
						}).catch(() => {
							update(oldGuild, newGuild)
							if (raidlog) return raidlog.send(new EmbedBuilder().setColor(color).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a modifier le serveur, mais il n'a pas pu être **derank** !`))
						})
					}
				}
			}
