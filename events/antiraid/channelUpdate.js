			...
			if (db.get(`channelsmod_${guild.id}`) === true && !perm) {
				if (db.get(`channelsmodsanction_${guild.id}`) === "ban") {
					axios({
						...
						if (raidlog) return raidlog.send(new EmbedBuilder().setColor(color).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a modifier le salon ${oldChannel}, il a été **ban** !`))
					}).catch(() => {

						if (raidlog) return raidlog.send(new EmbedBuilder().setColor(color).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a modifier le salon ${oldChannel}, mais il n'a pas pu être **ban** !`))
					...
					guild.members.cache.get(response.data.audit_log_entries[0].user_id).kick().then(() => {
						...
						if (raidlog) return raidlog.send(new EmbedBuilder().setColor(color).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a modifier le salon ${oldChannel}, il a été **kick** !`))
					}).catch(() => {

						if (raidlog) return raidlog.send(new EmbedBuilder().setColor(color).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a modifier le salon ${oldChannel}, mais il n'a pas pu être **kick** !`))
					})
					...
					guild.members.cache.get(response.data.audit_log_entries[0].user_id).roles.set([]).then(() => {
						...
						if (raidlog) return raidlog.send(new EmbedBuilder().setColor(color).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a modifier le salon ${oldChannel}, il a été **derank** !`))
					}).catch(() => {

						if (raidlog) return raidlog.send(new EmbedBuilder().setColor(color).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a modifier le salon ${oldChannel}, mais il n'a pas pu être **derank** !`))
					})
				}
