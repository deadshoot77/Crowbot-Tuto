			...
			if (ban <= banLimit) {
				...
				if (db.get(`massbansanction_${guild.id}`) === "ban") {
					axios({
						...
						if (raidlog) return raidlog.send(new EmbedBuilder().setColor(color).setDescription(`<@${action.executor.id}> a ban ${member}, il a été **ban** !`))
					}).catch(() => {

						if (raidlog) return raidlog.send(new EmbedBuilder().setColor(color).setDescription(`<@${action.executor.id}> a ban ${member}, mais il n'a pas pu être **ban** !`))

					})
				} else if (db.get(`massbansanction_${guild.id}`) === "kick") {
					guild.members.cache.get(action.executor.id).kick().then(() => {

						if (raidlog) return raidlog.send(new EmbedBuilder().setColor(color).setDescription(`<@${action.executor.id}> a ban ${member}, il a été **kick** !`))
					}).catch(() => {

						if (raidlog) return raidlog.send(new EmbedBuilder().setColor(color).setDescription(`<@${action.executor.id}> a ban ${member}, mais il n'a pas pu être **kick** !`))
					})
					...
					guild.members.cache.get(action.executor.id).roles.set([]).then(() => {
						...
						if (raidlog) return raidlog.send(new EmbedBuilder().setColor(color).setDescription(`<@${action.executor.id}> a ban ${member}, il a été **derank** !`))
					}).catch(() => {

						if (raidlog) return raidlog.send(new EmbedBuilder().setColor(color).setDescription(`<@${action.executor.id}> a ban ${member}, mais il n'a pas pu être **derank** !`))
					})
				}
			}
